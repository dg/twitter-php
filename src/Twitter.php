...
class Twitter
{
	 ...
	 * @param string media
	 *   Path to local media file to be uploaded
	 * @param array $options
	 *   Any additional options to send to statuses/update
	 *
    ...
    $request = array_merge($options, ['status' => $message, 'media_ids' => implode(',', $mediaIds) ?: NULL]);
		return $this->request(
			'statuses/update',
			'POST',
			$request
